...
// Illustrative blended cost per inbound+outbound message (WhatsApp + LLM + STT/TTS).
// See docs/SUSTAINABILITY_B2G.md — replace with metered usage when available.
...
export async function programMetrics(): Promise<ProgramMetrics | null> {
  ...
  const [pc, mc, alertsRes, refRes, apptRes] = await Promise.all([
  ...
  const estMonthlyCostUsd = Math.round(totalMessages * COST_PER_MESSAGE_USD * 100) / 100;
