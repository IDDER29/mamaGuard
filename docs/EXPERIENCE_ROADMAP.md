...
**Phase E2 — conversation:** ✅ E2.1 command intents + keywords (STOP/HELP/LANG, unit-tested) · ✅ E2.2 language switch end-to-end · ✅ E2.3 content translated (Darija/FR/AR; FR/AR machine-drafted, pending clinician review) · ✅ E2.4 channel layer hardened (retry+backoff, `message_deliveries` logging, graceful degradation; SMS needs Twilio creds, USSD/voice remain stubs).
**Phase E3 — dashboard UX:** 🟦 E3.1 primitives built (`StateViews`, badges, error boundary); full card refactor not pursued (low value/high churn) · ✅ E3.2 mobile bottom nav · 🟦 E3.3 a11y (modal dialog/Esc done; full WCAG audit recommended) · ✅ E3.4 dashboard i18n + RTL (framework + EN/FR/AR nav + dir=rtl; string coverage extends incrementally) · ✅ E3.5 live SLA tick + owner visibility.
**Phase E4 — roles & admin:** ✅ E4.1 role model + profiles · ✅ E4.2 invite flow (create/accept + admin form) · ✅ E4.3 admin console · ✅ E4.4 supervisor team views (+ reassign actions).
**Phase E5 — notifications/settings/states:** ✅ E5.1 notifications center · ✅ E5.2 settings · 🟦 E5.3 reminder opt-out (STOP/START global opt-out live; per-category pending) · ✅ E5.4 error boundary + offline banner + empty states.
**Phase E6 — compliance:** ✅ E6.1 consent withdrawal/restore (STOP/START) · ✅ E6.2 right-to-erasure + retention purge · ⬜ E6.3 data residency (legal/infra — see `COMPLIANCE.md`) · ✅ E6.4 identity/merge (validated on Postgres; admin control).
**Phase E7 — reporting:** ✅ E7.1 program + illustrative cost KPI · ✅ E7.2 scheduled export (weekly cron → REPORT_WEBHOOK_URL) · ✅ E7.3 outcome metrics.
**Phase E8 — strategic seams (production-ready, external-gated):** ✅ E8.1 teleconsult link (needs a video provider link) · ✅ E8.2 predictive risk heuristic · ✅ E8.3 device/wearable ingestion API (`/api/ingest/vitals`, needs `INGEST_TOKEN`) · ✅ E8.4 PWA foundation (manifest + offline SW) · ✅ E8.5 fine-tuned-model provider seam (`MAMALLAMA_*`, needs a hosted model) · ✅ E8.6 FHIR write-back (`/api/fhir/observation`, needs `FHIR_TOKEN`/EHR).

**The software for every roadmap item is now implemented and CI-verified.** What remains is genuinely external and cannot live in this repo: a **Twilio/USSD account** (SMS/USSD live traffic), **clinician review** of the FR/AR translations, **legal/CNDP sign-off + Supabase region** (E6.3), a **video-consult provider** link, **BP-cuff/wearable hardware**, a **hosted fine-tuned model**, and a **national-EHR endpoint** — each wired behind an env var / link field so it works the moment the credential is supplied. Migrations `0000`–`0009` validated (162 cols, idempotent); CI gates lint/type/build + triage vignettes + under-triage safety + conversation parser. See `PRODUCTION_READINESS.md`.
